...
class PaymentRequest extends AbstractRequest
{
    ...
    {
        $httpResponse = $this->httpClient->request('GET', $this->getEndpoint() . '/create/?' . http_build_query(array_intersect_key($data, array_flip(['callback', 'address', 'pending', 'confirmations', 'email', 'post', 'priority']))));
        if ($httpResponse->getStatusCode() != 200) {
            throw new RuntimeException('Unexpected cryptapi response!');
        }
        ...
        $httpResponse = $this->httpClient->request('GET', $this->getEndpoint() . '/qrcode/?' . http_build_query(['address' => $responseData->address_in, 'value' => $this->getAmount()]));
        if ($httpResponse->getStatusCode() != 200) {
            throw new RuntimeException('Unexpected cryptapi response!');
        }
